"""Shared helpers + theme — orientation-agnostic CORE (python_dark template).

A DARK code-editor look for programming tutorials: a near-black canvas, a faint
editor grid, syntax-highlighted code on a window-framed card, and TYPING
animations (typewriter reveal for text and code). The palette uses Python's blue
+ yellow as accents.

This is a FRAGMENT: scenes.SceneSynthesizer.materialize_dir composes a build's
_common.py from an orientation delta (_landscape.py / _portrait.py) spliced in at
the ORIENTATION_DELTA marker, followed by the active subject pack's helpers. The
delta defines the size constants + frame config used below, so this file is not
importable on its own.

Typing helpers (the headline feature of this template):
  * ``type_text(mob, cursor=caret())`` -> an Animation that types a Text/MarkupText
    one glyph at a time (optionally trailing a blinking-style block caret). Play
    it with ``self.play(type_text(body_text("...")))``.
  * ``type_code(self, card)`` -> reveals a ``code_card`` window instantly, then
    types the SYNTAX-HIGHLIGHTED code character by character. Drives the scene
    itself, so just call ``type_code(self, code_card(src))``.
"""
...
# --- Dark code-editor palette --------------------------------------------
# Near-black IDE canvas, light text, Python blue/yellow accents, deep code
# surfaces. Every helper below reads these constants, so the whole look is
# re-themed from this one block.
PRIMARY = "#4B8BBE"      # Python blue — titles / structure
ACCENT = "#58A6FF"       # bright blue — section labels, top rule
HIGHLIGHT = "#FFD43B"    # Python yellow — emphasis, bullets, caret
BG = "#0D1117"           # editor background (near-black)
TEXT = "#E6EDF3"         # primary light text
DANGER = "#F85149"       # errors / anti-patterns
OK = "#3FB950"           # success / correct output
CARD_BG = "#161B22"      # raised panel / card surface
CARD_SHADOW = "#010409"  # drop shadow (darker than the page)
CODE_BG = "#0B1020"      # code-card interior (deep)
CODE_BORDER = "#30363D"  # code-card / panel border
PANEL_DARK = "#010409"   # title bar / UML header (darkest)
MUTED = "#8B949E"        # comments / secondary text
GRID = "#1B2230"         # faint editor grid lines
...
# --- Typography helpers ---------------------------------------------------
# Title / heading: Noto Sans Bold (light blue on dark). Body: Noto Sans Regular
# (light grey). Section labels: Noto Sans SemiBold (bright blue).
...
def mono_text(text: str, size: int = BODY_SIZE, color=TEXT,
              weight=W_REGULAR) -> Text:
    """A plain monospace line (JetBrains Mono) — handy for inline code, REPL
    prompts, or anything you want to TYPE with ``type_text`` so the typewriter
    cadence reads like a terminal."""
    return Text(text, font=CODE_FONT, font_size=size, color=color, weight=weight)


# --- Scene surface --------------------------------------------------------

def tech_background() -> VGroup:
    """Dark editor canvas with a faint code grid behind every scene."""
    fw, fh = config.frame_width, config.frame_height
    ...
    lines.set_stroke(GRID, width=1, opacity=0.5)
...
def soft_panel(width: float, height: float, stroke=CODE_BORDER, fill=CARD_BG,
...
def node_label(text: str, color=ACCENT, width: float = NODE_W,
    ...
    box = RoundedRectangle(corner_radius=NODE_RADIUS, width=width, height=height,
                           ...
                           fill_color=fill, fill_opacity=1.0)
    label = MarkupText(text, font_size=font_size, color=TEXT,
...
# --- Syntax-highlighted code card (window editor) -------------------------

def code_card(code_str: str, language: str = "python",
              font_size: int = 24, max_width: float | None = None,
              max_height: float | None = None,
              line_spacing: float = 0.6) -> Code:
    """A window-framed, dark, syntax-highlighted code editor card.

    Defaults to ``language="python"``. Reveal it with the typewriter effect via
    ``type_code(self, code_card(src))`` — the window chrome appears at once and
    the code is typed in character by character."""
    card = Code(
    ...
    bg = getattr(card, "background", None)
    if bg is not None:
        shadow = RoundedRectangle(corner_radius=0.16,
                                  width=bg.width,
                                  height=bg.height,
                                  stroke_opacity=0,
                                  fill_color=CARD_SHADOW,
                                  fill_opacity=0.55)
        shadow.move_to(bg).shift(RIGHT * 0.07 + DOWN * 0.07)
...
#   * type_code(self, card)           -> drives the scene to type a code_card

def caret(height: float | None = None, color=HIGHLIGHT) -> Rectangle:
    """A solid block caret to trail a typewriter reveal (pass as
    ``type_text(mob, cursor=caret())``)."""
    h = CARET_H if height is None else height
    return Rectangle(width=0.5 * h, height=h, fill_color=color,
                     fill_opacity=1.0, stroke_opacity=0)


def type_text(mob: Mobject, run_time: float | None = None,
              time_per_char: float = 0.05, cursor: Mobject | None = None,
              **kwargs) -> Animation:
    """Typewriter-reveal a Text / MarkupText one glyph at a time.

    Play it directly: ``self.play(type_text(body_text("Hello")))``. Pass a
    ``cursor`` mobject (e.g. ``caret()``) to trail a blinking-style block caret
    via Manim's TypeWithCursor. ``run_time`` overrides the per-char pacing."""
    kw = dict(kwargs)
    if run_time is not None:
        kw["run_time"] = run_time
    else:
        kw["time_per_char"] = time_per_char
    if cursor is not None:
        return TypeWithCursor(mob, cursor, **kw)
    return AddTextLetterByLetter(mob, **kw)


def _glyph_leaves(mob: Mobject):
    """Drawn glyph leaves of `mob`, in reading order (a Code line is a VGroup of
    word-VGroups, so AddTextLetterByLetter can't take it — we reveal the leaves
    with ShowIncreasingSubsets instead, which keeps the syntax colors)."""
    return [m for m in mob.get_family() if not m.submobjects and m.has_points()]


def _code_glyphs(card: Code):
    lines = getattr(card, "code_lines", None)
    return _glyph_leaves(lines if lines is not None else card)


def type_code(scene: Scene, card: Code, run_time: float | None = None,
              time_per_char: float = 0.03) -> Code:
    """Typewriter-reveal a `code_card` ON `scene`: draw the window chrome at once,
    then type the syntax-highlighted code character by character.

    Usage:
        card = code_card("print('hi')").to_edge(LEFT)
        type_code(self, card)          # card is a single mobject on screen after

    Returns the card so you can keep animating it (highlight lines, shift, …)."""
    glyphs = VGroup(*_code_glyphs(card))
    chrome = VGroup(*[m for m in card.submobjects
                      if m is not getattr(card, "code_lines", None)])
    if len(chrome) > 0:
        scene.add(chrome)          # window frame + dots + shadow appear instantly
    n = max(1, len(glyphs))
    rt = run_time if run_time is not None else min(7.0, 0.5 + n * time_per_char)
    scene.play(ShowIncreasingSubsets(glyphs, run_time=rt))
    # Reattach as one unit so later self.play(card.animate...) / layout checks
    # see a single top-level mobject, not the transient chrome/glyph groups.
    scene.remove(chrome, glyphs)
    scene.add(card)
    return card
