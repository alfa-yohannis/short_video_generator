"""Shared helpers + theme for portrait (9:16) scenes."""
...
def tech_background() -> VGroup:
    """Subtle blueprint grid used behind every portrait scene."""
    fw, fh = config.frame_width, config.frame_height
    ...
    top_rule = Rectangle(width=fw, height=0.05, fill_color=ACCENT,
                         fill_opacity=0.95, stroke_opacity=0)
    top_rule.to_edge(UP, buff=0)
    safe_rule = DashedLine([-fw / 2 + 0.35, SHORTS_SAFE_BOTTOM, 0],
                           [fw / 2 - 0.35, SHORTS_SAFE_BOTTOM, 0],
                           color=GRID, dash_length=0.18,
                           stroke_width=1.4)
    safe_rule.set_opacity(0.35)
    bg = VGroup(base, lines, top_rule, safe_rule)
    bg.set_z_index(-20)
    return bg


def soft_panel(width: float, height: float, stroke=PRIMARY, fill=CARD_BG,
               radius: float = 0.18, stroke_width: float = 2,
...
def node_label(text: str, color=PRIMARY, width: float = 4.8,
               height: float = 0.82, font_size: int = 25,
    ...
    if label.width > width - 0.45:
        label.scale((width - 0.45) / label.width)
...
def title_bar(text: str, eyebrow: str = "") -> VGroup:
    ...
    if label.width > config.frame_width - 0.9:
        ...
        label.align_to(bar, LEFT).shift(RIGHT * 0.45)
    return VGroup(bar, left_accent, accent_strip, label)


def bullet_list(items, size: int = 32, dot_color=HIGHLIGHT,
                text_color=TEXT, buff_v: float = 0.45,
                buff_h: float = 0.3) -> VGroup:
    rows = VGroup()
    for s in items:
        dot = RoundedRectangle(corner_radius=0.04, width=0.22, height=0.22,
...
def uml_class_box(name: str, stereotype: str | None = None,
                  ...
                  width: float = 6.5, header_color=None) -> VGroup:
...
def _hollow_triangle(tip, base_center, color=PRIMARY, size: float = 0.32):
...
def association_arrow(start, end, color=PRIMARY,
    ...
    group = VGroup(arrow)
    if label:
        lbl = MarkupText(label, font_size=22, color=color, slant=ITALIC)
        mid = (np.array(start) + np.array(end)) / 2
        direction = np.array(end) - np.array(start)
        if np.linalg.norm(direction) > 0:
            unit = direction / np.linalg.norm(direction)
            perp = np.array([-unit[1], unit[0], 0]) * 0.35
